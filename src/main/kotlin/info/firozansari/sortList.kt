...
fun sortList(sortNumber: Int, sortOrder: Int, sortList_before: MutableList<Student>) {
    ...
    when (sortNumber) {
        ...
        2 -> {
            if (sortInAscendingOrder) {
                val sortedList = sortList_before.sortedBy { it.age }
                fileName = "ascending.csv"
                csvFileWrite(fileName, sortedList)
            } else {
                val sortedList = sortList_before.sortedByDescending { it.age }
                fileName = "descending.csv"
                csvFileWrite(fileName, sortedList)
            }
        }
        ...
        3 -> {
            if (sortInAscendingOrder) {
                val sortedList = sortList_before.sortedBy { it.score }
                fileName = "ascending.csv"
                csvFileWrite(fileName, sortedList)
            } else {
                val sortedList = sortList_before.sortedByDescending { it.score }
                fileName = "descending.csv"
                csvFileWrite(fileName, sortedList)
            }
        }
